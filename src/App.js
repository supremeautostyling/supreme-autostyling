...
import Sharer from "./components/sharer/Sharer.js";
...
const efx = {
  ...
  route: {
    ...
    atLeave: { scale: 0 },
    atActive: { scale: 1 },
    mapStyles: (styles) => ({ transform: `scale(${styles.scale})` }),
  },
};
...
class App extends Component {
  ...
  render() {
    const { showScrollBtn, activeNav, shareBtns, isMobile } = this.state;
    const atRoot = this.props.location.pathname === "/";
    return (
      <>
        <NavBar activeNav={activeNav} changeRoute={this.changeRoute} />

        <Header isMobile={isMobile} greet={false} />

        {GoogleAnalytics.init() && <GoogleAnalytics.RouteTracker />}

        <AnimatedSwitch {...efx.switch} className={`switch-wrapper ${atRoot ? "" : "h-100"}`}>
          <AnimatedRoute
            exact
            path="/"
            {...efx.route}
            render={(props) => <Home {...props} setRef={this.setRef} isMobile={isMobile} />}
          />
